...
tNFA_STATUS NFA_P2pRegisterServer(uint8_t server_sap,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s - server_sap:0x%02x, link_type:0x%x, SN:<%s>",
                      __func__, server_sap, link_type, p_service_name);

  if ((server_sap != NFA_P2P_ANY_SAP) &&
      ((server_sap <= LLCP_SAP_SDP) ||
       (server_sap > LLCP_UPPER_BOUND_SDP_SAP))) {
    LOG(ERROR) << StringPrintf("%s - server_sap must be between %d and %d",
                               __func__, LLCP_SAP_SDP + 1,
...
tNFA_STATUS NFA_P2pDeregister(tNFA_HANDLE handle) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s - handle:0x%02X", __func__, handle);

  xx = handle & NFA_HANDLE_MASK;

  if ((xx >= NFA_P2P_NUM_SAP) || (nfa_p2p_cb.sap_cb[xx].p_cback == nullptr)) {
    LOG(ERROR) << StringPrintf("%s - Handle is invalid or not registered",
                               __func__);
...
tNFA_STATUS NFA_P2pAcceptConn(tNFA_HANDLE handle, uint16_t miu, uint8_t rw) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s - handle:0x%02X, MIU:%d, RW:%d", __func__, handle, miu, rw);

  xx = handle & NFA_HANDLE_MASK;

  if (!(xx & NFA_P2P_HANDLE_FLAG_CONN)) {
    LOG(ERROR) << StringPrintf("%s - Connection Handle is not valid", __func__);
    return (NFA_STATUS_BAD_HANDLE);
  } else {
  ...
  if ((miu < LLCP_DEFAULT_MIU) || (nfa_p2p_cb.local_link_miu < miu)) {
    LOG(ERROR) << StringPrintf("%s - MIU(%d) must be between %d and %d",
                               __func__, miu, LLCP_DEFAULT_MIU,
                               nfa_p2p_cb.local_link_miu);
...
tNFA_STATUS NFA_P2pDisconnect(tNFA_HANDLE handle, bool flush) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s - handle:0x%02X, flush=%d", __func__, handle, flush);
  ...
  if (xx & NFA_P2P_HANDLE_FLAG_CONN) {
    ...
    if ((xx >= LLCP_MAX_DATA_LINK) || (nfa_p2p_cb.conn_cb[xx].flags == 0)) {
      LOG(ERROR) << StringPrintf("%s - Connection Handle is not valid",
                                 __func__);
      return (NFA_STATUS_BAD_HANDLE);
    }
  } else {
    LOG(ERROR) << StringPrintf("%s - Handle is not valid", __func__);
    return (NFA_STATUS_BAD_HANDLE);
  }
...
tNFA_STATUS NFA_P2pConnectByName(tNFA_HANDLE client_handle,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s - client_handle:0x%x, SN:<%s>, MIU:%d, RW:%d",
                      __func__, client_handle, p_service_name, miu, rw);

  xx = client_handle & NFA_HANDLE_MASK;

  if ((xx >= NFA_P2P_NUM_SAP) || (nfa_p2p_cb.sap_cb[xx].p_cback == nullptr)) {
    LOG(ERROR) << StringPrintf("%s - Client Handle is not valid", __func__);
    return (NFA_STATUS_BAD_HANDLE);
  }
  ...
  if ((miu < LLCP_DEFAULT_MIU) ||
    ...
    LOG(ERROR) << StringPrintf(
        "%s - MIU(%d) must be between %d and %d or LLCP "
        "link is not activated",
        __func__, miu, LLCP_DEFAULT_MIU, nfa_p2p_cb.local_link_miu);
...
tNFA_STATUS NFA_P2pConnectBySap(tNFA_HANDLE client_handle, uint8_t dsap,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s - client_handle:0x%x, DSAP:0x%02X, MIU:%d, RW:%d",
                      __func__, client_handle, dsap, miu, rw);
...
tNFA_STATUS NFA_P2pSendUI(tNFA_HANDLE handle, uint8_t dsap, uint16_t length,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s - handle:0x%X, DSAP:0x%02X, length:%d", __func__,
                      handle, dsap, length);
  ...
  if ((xx >= NFA_P2P_NUM_SAP) || (nfa_p2p_cb.sap_cb[xx].p_cback == nullptr)) {
    LOG(ERROR) << StringPrintf("%s - Handle (0x%X) is not valid", __func__,
                               handle);
    ret_status = NFA_STATUS_BAD_HANDLE;
  } else if (length > nfa_p2p_cb.remote_link_miu) {
    LOG(ERROR) << StringPrintf(
        "%s - handle:0x%X, length(%d) must be less than remote "
        "link MIU(%d)",
        __func__, handle, length, nfa_p2p_cb.remote_link_miu);
    ret_status = NFA_STATUS_BAD_LENGTH;
  } else if (nfa_p2p_cb.sap_cb[xx].flags & NFA_P2P_SAP_FLAG_LLINK_CONGESTED) {
    LOG(WARNING) << StringPrintf(
        "%s - handle:0x%X, logical data link is already congested", __func__,
        handle);
    ...
    LOG(WARNING) << StringPrintf(
        "%s - handle:0x%X, logical data link is congested", __func__, handle);
    ret_status = NFA_STATUS_CONGESTED;
...
tNFA_STATUS NFA_P2pSendData(tNFA_HANDLE handle, uint16_t length,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s - handle:0x%X, length:%d", __func__, handle, length);
  ...
  if ((!(handle & NFA_P2P_HANDLE_FLAG_CONN)) || (xx >= LLCP_MAX_DATA_LINK) ||
    ...
    LOG(ERROR) << StringPrintf(
        "%s - handle:0x%X, Remote set RW to 0 (flow off)", __func__, handle);
    ret_status = NFA_STATUS_FAILED;
  } else if (nfa_p2p_cb.conn_cb[xx].remote_miu < length) {
    LOG(ERROR) << StringPrintf(
        "%s - handle:0x%X, Data more than remote MIU(%d)", __func__, handle,
        nfa_p2p_cb.conn_cb[xx].remote_miu);
    ret_status = NFA_STATUS_BAD_LENGTH;
  } else if (nfa_p2p_cb.conn_cb[xx].flags & NFA_P2P_CONN_FLAG_CONGESTED) {
    LOG(WARNING) << StringPrintf(
        "%s - handle:0x%X, data link connection is already "
        "congested",
        __func__, handle);
    ret_status = NFA_STATUS_CONGESTED;
    ...
    LOG(WARNING) << StringPrintf(
        "%s - handle:0x%X, data link connection is congested", __func__,
        handle);
    ret_status = NFA_STATUS_CONGESTED;
...
tNFA_STATUS NFA_P2pSetLocalBusy(tNFA_HANDLE conn_handle, bool is_busy) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s - conn_handle:0x%02X, is_busy:%d", __func__, conn_handle, is_busy);
...
tNFA_STATUS NFA_P2pGetLinkInfo(tNFA_HANDLE handle) {
  ...
  if (nfa_p2p_cb.llcp_state != NFA_P2P_LLCP_STATE_ACTIVATED) {
    LOG(ERROR) << StringPrintf("%s - LLCP link is not activated", __func__);
    return (NFA_STATUS_FAILED);
  }
...
tNFA_STATUS NFA_P2pGetRemoteSap(tNFA_HANDLE handle, char* p_service_name) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s - handle:0x%x, SN:<%s>", __func__, handle, p_service_name);
...
tNFA_STATUS NFA_P2pSetLLCPConfig(uint16_t link_miu, uint8_t opt, uint8_t wt,
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "                       %s - inact_timeout(init:%d, target:%d), "
  ...
  if (nfa_p2p_cb.llcp_state == NFA_P2P_LLCP_STATE_ACTIVATED) {
    LOG(ERROR) << StringPrintf("%s - LLCP link is activated", __func__);
    return (NFA_STATUS_FAILED);
  }
...
void NFA_P2pGetLLCPConfig(uint16_t* p_link_miu, uint8_t* p_opt, uint8_t* p_wt,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s -                        delay_first_pdu_timeout:%d",
                      __func__, *p_delay_first_pdu_timeout);
}

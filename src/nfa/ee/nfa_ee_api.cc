...
tNFA_STATUS NFA_EeDiscover(tNFA_EE_CBACK* p_cback) {
  ...
  if (nfa_ee_cb.em_state != NFA_EE_EM_STATE_INIT_DONE) {
    ...
    LOG(ERROR) << StringPrintf("%s - in progress or NULL callback function",
                               __func__);
...
tNFA_STATUS NFA_EeGetInfo(uint8_t* p_num_nfcee, tNFA_EE_INFO* p_info) {
  ...
  *p_num_nfcee = 0;
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s - num_ret:%d", __func__, num_ret);
  *p_num_nfcee = num_ret;
...
tNFA_STATUS NFA_EeModeSet(tNFA_HANDLE ee_handle, tNFA_EE_MD mode) {
  ...
  if (p_found == nullptr) {
    LOG(ERROR) << StringPrintf("%s - invalid NFCEE:0x%04x", __func__,
                               ee_handle);
    status = NFA_STATUS_INVALID_PARAM;
  } else {
...
tNFA_STATUS NFA_EeSetDefaultTechRouting(
  ...
  if (p_cb == nullptr) {
    LOG(ERROR) << StringPrintf("%s - Bad ee_handle", __func__);
    status = NFA_STATUS_INVALID_PARAM;
  } else {
    p_msg =
...
tNFA_STATUS NFA_EeClearDefaultTechRouting(
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s - handle:<0x%x>clear technology_mask:<0x%x>",
...
tNFA_STATUS NFA_EeAddAidRouting(tNFA_HANDLE ee_handle, uint8_t aid_len,
  ...
  if ((p_cb == nullptr) ||
...
#ifdef ST21NFC
    // Store data for further processing of this is default AID
    if (aid_len == 0) {
      tNFA_EE_API_DEFAULT_AID_ROUTE* p_msg2;
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s - Default AID route 0x%02X", __func__, nfcee_id);

      if ((p_msg2 = (tNFA_EE_API_DEFAULT_AID_ROUTE*)GKI_getbuf(
               sizeof(tNFA_EE_API_DEFAULT_AID_ROUTE))) != NULL) {
        p_msg2->hdr.event = NFA_EE_API_DEFAULT_AID_ROUTE_EVT;
        p_msg2->nfcee_id = nfcee_id;
        p_msg2->p_cb = p_cb;
        p_msg2->aid_len = aid_len;
        p_msg2->power_state = power_state;
        p_msg2->p_aid = (uint8_t*)(p_msg2 + 1);
        p_msg2->aidInfo = aidInfo;
        nfa_sys_sendmsg(p_msg2);
      }
    ...
    if (p_msg != nullptr) {
      if (p_aid != nullptr)
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "%s - aid:<%02x%02x>", __func__, p_aid[0], p_aid[1]);
...
tNFA_STATUS NFA_EeConnect(tNFA_HANDLE ee_handle, uint8_t ee_interface,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s - handle:<0x%x> ee_interface:0x%x", __func__,
                      ee_handle, ee_interface);
  p_cb = nfa_ee_find_ecb(nfcee_id);

  if ((p_cb == nullptr) || (p_cback == nullptr)) {
    LOG(ERROR) << StringPrintf("%s - Bad ee_handle or NULL callback function",
                               __func__);
    status = NFA_STATUS_INVALID_PARAM;
  } else {
...
tNFA_STATUS NFA_EeSendData(tNFA_HANDLE ee_handle, uint16_t data_len,
  ...
  if ((p_cb == nullptr) || (p_cb->conn_st != NFA_EE_CONN_ST_CONN) ||
      (p_data == nullptr)) {
    LOG(ERROR) << StringPrintf("%s - Bad ee_handle or NULL data", __func__);
    status = NFA_STATUS_INVALID_PARAM;
  } else {
...
**
*******************************************************************************/
tNFA_STATUS NFA_EePowerCtrl(tNFA_HANDLE ee_handle, uint8_t config) {
  tNFA_EE_API_POWER_CTRL* p_msg;
  tNFA_STATUS status = NFA_STATUS_FAILED;
  uint8_t nfcee_id = (uint8_t)(ee_handle & 0xFF);
  tNFA_EE_ECB* p_cb;

  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_EePowerCtrl(): handle:<0x%x>", ee_handle);
  p_cb = nfa_ee_find_ecb(nfcee_id);

  if (p_cb == nullptr) {
    LOG(ERROR) << StringPrintf("NFA_EePowerCtrl() Bad ee_handle");
    status = NFA_STATUS_INVALID_PARAM;
  } else if ((p_msg = (tNFA_EE_API_POWER_CTRL*)GKI_getbuf(
                  sizeof(tNFA_EE_API_POWER_CTRL))) != NULL) {
    p_msg->hdr.event = NFA_EE_API_POWER_CTRL_EVT;
    p_msg->nfcee_id = nfcee_id;
    p_msg->p_cb = p_cb;
    p_msg->config = config;
...
**
** Description      Force the routing to a particular NFCEE. The result of this
**                  operation is reported with the NFA_EE_FORCE_ROUTING_EVT.
**
** Returns          NFA_STATUS_OK if successful
**
*******************************************************************************/
tNFA_STATUS NFA_EeForceRouting(tNFA_HANDLE ee_handle, uint8_t config) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_EeForceRouting(): handle:<0x%x>", ee_handle);
  p_cb = nfa_ee_find_ecb(nfcee_id);

  if (p_cb == nullptr) {
    LOG(ERROR) << StringPrintf("NFA_EeForceRouting() Bad ee_handle");
    status = NFA_STATUS_INVALID_PARAM;
  } else if ((p_msg = (tNFA_EE_API_FORCE_ROUTING*)GKI_getbuf(
                  sizeof(tNFA_EE_API_FORCE_ROUTING))) != NULL) {
    p_msg->hdr.event = NFA_EE_API_FORCE_ROUTING_EVT;
    p_msg->nfcee_id = nfcee_id;
    p_msg->p_cb = p_cb;
    p_msg->config = config;
...
**
** Description      Stop forcing the routing to a particular NFCEE. The result
*of this
**                  operation is reported with the NFA_EE_FORCE_ROUTING_EVT.
**
** Returns          NFA_STATUS_OK if successful
**
*******************************************************************************/
tNFA_STATUS NFA_EeStopForceRouting() {
  tNFA_EE_API_FORCE_ROUTING* p_msg;
  tNFA_STATUS status = NFA_STATUS_FAILED;

  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("NFA_EeStopForceRouting()");

  if ((p_msg = (tNFA_EE_API_FORCE_ROUTING*)GKI_getbuf(
           sizeof(tNFA_EE_API_FORCE_ROUTING))) != NULL) {
    p_msg->hdr.event = NFA_EE_API_STOP_FORCE_ROUTING_EVT;
    p_msg->nfcee_id = 0;
    p_msg->p_cb = 0;
    p_msg->config = 0;
...
**
** Description      Clears all AID, SC, tech and protocol entries in RT
**
** Returns          NFA_STATUS_OK if successful
**
*******************************************************************************/
tNFA_STATUS NFA_EeClearRoutingTable(bool clear_sc) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("NFA_EeClearRoutingTable() - clear_sc: %d", clear_sc);
